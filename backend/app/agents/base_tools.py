...
class StrategyTool(AgentTool):
    ...
    def execute(self, context: AgentContext) -> Tuple[AgentTurn, Dict[str, Any]]:
        ...
        # heuristic fallback for keywords
        ...
        prompt = f"""
        You are the Strategy Planner for a legal AI system. Create a concise, 4-step execution plan to answer the user's request.
        
        User Request: "{context.question}"
        
        Available Capabilities:
        1. Ingestion Audit: Check document coverage.
        2. Legal Research: Retrieve relevant case law and statutes.
        3. Forensics Enrichment: Attach digital forensics artifacts to evidence.
        4. QA Adjudication: Evaluate the final answer against a rubric.
        
        Instructions:
        1. Generate 4 specific, sequential steps tailored to the request.
        2. Identify 3-5 key entities or legal concepts to focus on.
        3. Return ONLY a JSON object with keys: "steps" (list of strings) and "focus_entities" (list of strings).
        """
        ...
        try:
            ...
            if json_match:
                ...
                focus_entities = plan_data.get("focus_entities", unique_keywords)
            else:
                raise ValueError("No JSON found in response")
        except Exception as e:
            print(f"Strategy LLM failed, falling back to heuristics: {e}")
            steps = [
        ...
        # ... rest of existing logic for directives and graph ...
