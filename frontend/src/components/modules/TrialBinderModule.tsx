...
import { FolderOpen, Star, FileText, Plus, X, Search, Loader2 } from 'lucide-react';
        ...
        <div className="w-full h-full flex flex-col p-8 text-halo-text overflow-hidden">
            <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h2 className="text-2xl font-light text-halo-text uppercase tracking-wider">Trial Binder</h2>
                        <p className="text-halo-muted text-sm">Key evidence and organized exhibits</p>
                    </div>
                </div>
                <button
                    onClick={() => setIsAdding(true)}
                    className="flex items-center gap-2 px-4 py-2 bg-halo-cyan text-black rounded hover:bg-white transition-colors font-medium"
                >
                    <Plus size={18} />
                    ADD EXHIBIT
                </button>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar">
                {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-64 text-halo-muted opacity-50 border-2 border-dashed border-halo-border rounded-xl">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {binderDocs.map(doc => (
                            <div key={doc.id} className="halo-card group relative hover:border-halo-cyan transition-all">
                                <div className="flex items-start justify-between mb-4">
                                    <FileText size={32} className="text-halo-cyan opacity-80" />
                                    <button
                                        onClick={() => togglePin(doc)}
                                        className="text-halo-muted hover:text-red-400 transition-colors"
                                    >
                                        <X size={18} />
                                    </button>
                                </div>
                                ...
                                <div className="absolute inset-0 bg-halo-cyan/5 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none" />
                            </div>
                        ))}
                    </div>
                )}
            ...
            {isAdding && (
                <div className="absolute inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-8">
                    <div className="bg-halo-bg border border-halo-cyan rounded-xl w-full max-w-2xl h-[80%] flex flex-col shadow-[0_0_50px_rgba(0,240,255,0.2)]">
                        ...
                        <div className="flex-1 overflow-y-auto p-4 custom-scrollbar space-y-2">
                            {filteredAllDocs.map(doc => (
                                <div
                                    key={doc.id}
                                    className="flex items-center justify-between p-3 rounded hover:bg-halo-cyan/10 border border-transparent hover:border-halo-cyan/30 transition-all group"
                                >
